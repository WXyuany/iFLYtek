                ...
                if (sub == "nlp" || sub == "iat" || sub == "tts" || sub == "asr" ||
                    ...
                    if (sub == "tts") {
                        ...
                        if (isUrl.asString() == "1") {
                            ...
                            std::fstream tts("tts.pcm", std::ios::app | std::ios::binary);
                            tts.write(buffer, dataLen);
                        }
                    } else if (sub == "iat") {
                        ...
                        if (reader.parse(resultStr, resultJson, false)) {
                            ...
                            if (resultJson.isMember("intent") &&
                                ...
                                const auto& intent = resultJson["intent"];

                                cout << sub << ": " << intent["text"].asString() << endl;

                                if (intent["rc"].asInt() == 0 && intent.isMember("answer")) {
                                    cout << "answer: " << intent["answer"]["text"].asString()
                                         << endl;
                                }

                            } else if (resultJson.isMember("text")) {
                                if (reader.parse(
                                        resultJson["text"].asString(), resultJson, false)) {
                                    cout << sub << ": " << resultJson["text"].asString() << endl;
                                }
                            }
                        }
                    } else {
        ...
        if (testData.is_open()) {
            ...
            while (testData.good() && !testData.eof()) {
                ...
                IAIUIMessage* writeMsg = IAIUIMessage::create(
                    AIUIConstant::CMD_WRITE, 0, 0, "data_type=audio", textData);
            ...
            IAIUIMessage* msg =
                IAIUIMessage::create(AIUIConstant::CMD_STOP_WRITE, 0, 0, "data_type=audio");
...
void writeText()
{
    if (NULL != agent) {
        ...
        IAIUIMessage* writeMsg =
            IAIUIMessage::create(AIUIConstant::CMD_WRITE, 0, 0, "data_type=text", textData);
...
void testTTs2()
{
    if (NULL != agent) {
        ...
        IAIUIMessage* ttsMsg =
            IAIUIMessage::create(AIUIConstant::CMD_TTS,
                                 1,
                                 0,
                                 "text_encoding=utf-8,vcn=xiaoyan,tts_res_type=url",
                                 textData);
